...
| 25    | (this commit)| **Group access = viewer baseline; per-user elevation; `admin`→`tenant_admin`**. Reworks how AD/Okta access is granted to fit the common case "everyone in this group should get in, but only one or two should administer". A group mapping no longer carries a role: membership only ever **admits a user into a tenant at the baseline `viewer` role**, and the Access control page drops the role picker + the platform_admin-via-group control (it's now group→tenant only). All elevation - operator, tenant_admin, platform_admin - happens **per-user in Members** (which already pins via `role_locked`). `pick_grant`/`resolve_grant` rewritten: first matched tenant-scoped group wins its tenant at viewer, tenant-less rows skipped, unmapped default restricted to none/viewer. The tenant-confined role is renamed **`admin`→`tenant_admin`** everywhere (ROLE_RANK, `require_admin`, credential/pipeline access predicates, CLI, templates) to disambiguate it from cross-tenant `platform_admin`; UI shows "Tenant admin". Idempotent data migration: rename `admin`→`tenant_admin` users, collapse tenant-scoped group mappings to `viewer`, delete retired tenant-less platform_admin mappings. Tests updated (okta resolver, rbac settings, tenancy, conftest) + new `test_role_migration.py`; full suite green (195, excl. FTP/pyOpenSSL env gap). |
| 26    | (this commit)| **Confluence listeners** - auto-sync Confluence spaces into the docs index. A per-tenant `ConfluenceListener` (table `confluence_listeners`) subscribes to one space; a background poller (`dosm/confluence/poller.py`, mirrors `pipelines.poller`, started in `main.py` startup, gated on `confluence.poller_enabled`) reconciles each enabled listener cross-tenant. `dosm/confluence/sync.py` pulls **pages** (Confluence storage HTML -> markdown via `markdownify`, optional `dosm[confluence]` extra, tag-strip fallback) and **attachments** (raw bytes) into the docs store under `confluence/<listener-slug>/`, writing via the existing `DocsStore` (local or SMB) so it lands wherever docs live, then `reindex_async` so the agent learns it right away. **Change detection + mirror-delete** via a `ConfluenceSyncItem` table (per-(listener,kind,confluence_id) version + rel_path): unchanged versions skip, removed items get their file deleted + de-indexed. Provider-pluggable async client (`ConfluenceClient` ABC + `CloudConfluenceClient` basic-auth email:token / `ServerConfluenceClient` Bearer PAT over `httpx`). Auth reuses a `login`/`pat` Credential (token in the secrets backend). Settings -> Confluence tab (list-CRUD + per-row Test / Sync now, mirrors cert sources); `dosm confluence list/add/rm/test/sync`; `AuditLog` on every mutation + sync. New tables auto-create (no migration). **Caveat**: downloaded docs land in the shared Default-tenant docs index (per-tenant docs roots are a later phase); REST paths are unvalidated against a live Confluence instance until smoke-tested. |
| 23    | (this branch)| **Host organisation - 3-tier tree** (branch `feature/host-org-tiers`). Groups hosts by **application -> environment -> (optional) unit** (e.g. Payments -> prod -> EU). New `OrgUnit` table (`org_units`): self-referencing `parent_id` (ON DELETE CASCADE so a subtree drops together), `tier` in {application, environment, unit}, `name`/`slug` unique per parent. The tier hierarchy (application has no parent; environment's parent is an application; unit's parent is an environment) is enforced in `dosm/applications/repo.py`, not the DB. `Host.org_unit_id` (idempotent column-add, ON DELETE SET NULL) points at the host's **deepest** placement; hosts roll up to coarser tiers by walking the subtree. New package `dosm/applications/` (router at `/applications`, nav item "Applications") - deliberately separate from `dosm.org` (the AD people directory) and the docs `Folder` taxonomy (which also uses table `applications`). Management UI: nested tree with inline add/edit/delete per node + rolled-up host counts; viewing is any-user, mutations are operator+ (mirrors host management), every mutation audit-logged (`orgunit.*`). Host form gains a cascading application/environment/unit picker (client-side filtered, posts the deepest node as `org_unit_id`); the inline edit row preserves the assignment via a hidden field so quick edits don't wipe it. Hosts list shows each host's org path, a "Application" search field, and a server-side subtree filter (`/hosts?org_unit_id=`). `dosm application tree/add/rename/rm/assign` (tier inferred from `--parent`; nodes addressable by id or `App/Env/Unit` path). Developed against an isolated `dosm-dev` compose stack (`docker-compose.dev.yml`, copy of the home dir, ports 8766/8889/11436) so it never touched the live inventory. |
...
### Phase 23 - Host organisation (3-tier tree)

The user wanted hosts grouped by **application -> environment -> unit**, where
the third tier (region/cell/etc.) is optional. Two shapes were considered:
three explicit FK columns on `Host`, or one self-referencing `OrgUnit` tree
(chosen). The tree won because environments like "prod" are *per-application*
(prod-of-Payments != prod-of-Billing) and a tree models that natively without
extra uniqueness gymnastics, and because it matches the existing
`Department`/`Host.jump_host` self-reference idiom.

Key decisions:
- **Host points at its deepest node**, not one column per tier. "All prod hosts
  in Payments" is a subtree walk (`org_subtree_ids`, <= 3 levels, one query per
  level - no recursive CTE needed). A host assigned only at the application tier
  is fine.
- **Tier rules live in the repo**, not the schema. SQLite can't express "parent
  must be exactly the tier above"; `create_unit` enforces it with clear errors.
  The only DB constraint is `UniqueConstraint(parent_id, name)` (root-level name
  collisions are caught in the repo since SQLite treats NULL parents as
  distinct).
- **Naming kept deliberately separate.** The package is `dosm/applications/`
  (route `/applications`, nav "Applications"), distinct from `dosm.org` (the
  AD-backed *people* directory) and the docs `Folder` taxonomy (which already
  owns the table named `applications`). The new table is `org_units`.
- **Adding a tier later** (a 4th level) would mean extending `TIERS` /
  `PARENT_TIER` / `CHILD_TIER` in `dosm/applications/repo.py` and the form's
  cascade JS - the tree itself imposes no depth limit.
